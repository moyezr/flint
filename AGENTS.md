...
3. Deploy migrations through `0004`, configure the verified Resend beta sender and OTP pepper, and deploy the hardened landing build plus ARM64 release metadata with production PostgreSQL, HTTPS, backups, monitoring, and abuse controls. Run both production verifiers and one restore drill against real infrastructure.

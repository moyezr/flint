...
4. Upload the current versioned DMG and checksum to a durable public artifact host. Beta 5 is temporarily shipped from `public/downloads`; use public object storage for repeated releases. Confirm `landing/app/lib/beta/latest-release.ts` or the `FLINT_BETA_*` environment variables match that artifact.
